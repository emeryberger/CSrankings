    ...
    for file in changed_lines:
        ...
        if matched:
            ...
            for l in changed_lines[file]:
                ...
                if re.search(",\\s", line):
                    print(
                        f"  ERROR: Found a space after a comma ({line}). Please ensure there are no spaces after commas."
                    )
                    valid, line_valid = (False, False)
                    continue
                try:
                    ...
                    if name[0].lower() != the_letter and the_letter != "0":
                        print(
                            f"  ERROR: This entry is in the wrong file. It is in `csrankings-{the_letter}.csv` but should be in `csrankings-{name[0].lower()}.csv`."
                        )
                        valid, line_valid = (False, False)

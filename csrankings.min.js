...
CSRankings.sum=function(a){for(var b=0,c=0;c<a.length;c++)b+=a[c];return b};CSRankings.average=function(a){return CSRankings.sum(a)/a.length};CSRankings.stddev=function(a){var b=CSRankings.average(a),c=a.map(function(d){d-=b;return d*d});return Math.sqrt(CSRankings.sum(c)/(a.length-1))};
...
CSRankings.prototype.inRegion=function(a,b){switch(b){case "us":if(a in this.countryInfo)return!1;break;case "au":case "br":case "ca":case "ch":case "cn":case "de":case "es":case "fr":case "il":case "in":case "it":case "jp":case "kr":case "nl":case "tr":case "uk":if(this.countryAbbrv[a]!=b)return!1;break;case "europe":if(!(a in this.countryInfo)||"europe"!=this.countryInfo[a])return!1;break;case "northamerica":if(a in this.countryInfo&&"canada"!=this.countryInfo[a])return!1;break;case "australasia":if(!(a in
...
n+'" onclick="trackOutboundLink(\''+n+"', true); return false;\">"+h.$jscomp$loop$prop$fc$28[l]+'</a></small></td><td align="right"><small>'+(Math.round(10*c[l])/10).toFixed(1)+'</small></td></tr><tr><td colspan="4"><div class="csr-piechart" id="'+escape(l)+'-chart"></div></td></tr>'}g+="</tbody></table></div>";f[e]=g}h={$jscomp$loop$prop$fc$28:h.$jscomp$loop$prop$fc$28}}return f};

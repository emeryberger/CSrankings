...
(this.authorAreas[e][m]=0)}this.authorAreas[c][f]+=g;this.authorAreas[e][f]+=g}}};
CSRankings.prototype.buildDepartments=function(a,b,d,c,f,e,g,k){var m={},h;for(h in this.authors)if(this.authors.hasOwnProperty(h)){var l=this.authors[h],n=l.name,q=l.year,p=l.area;l=l.dept;n in this.aliases&&(n=this.aliases[n]);if("undefined"!==typeof l&&!(0===d[p]||q<a||q>b)&&this.inRegion(l,c)){p in CSRankings.parentMap&&(p=CSRankings.parentMap[p]);q=p+l;p=n+l;q in this.areaDeptAdjustedCount||(this.areaDeptAdjustedCount[q]=0);var t=parseInt(this.authors[h].count),r=parseFloat(this.authors[h].adjustedcount);
this.areaDeptAdjustedCount[q]+=r;n in m||(m[n]=!0,g[p]=0,k[p]=0,l in f||(f[l]=0,e[l]=[]),e[l].push(n),f[l]+=1);g[p]+=t;k[p]+=r}}};
...
{};for(var m=$jscomp.makeIterator(a[e.dept]),h=m.next();!h.done;h=m.next())h=h.value,e.fc[h]=b[h+e.dept];m=Object.keys(e.fc);m.sort(function(a){return function(b,e){if(a.fc[e]===a.fc[b]){var f=Math.round(10*d[e+a.dept])/10,g=Math.round(10*d[b+a.dept])/10;return f===g?c.compareNames(b,e):f-g}return a.fc[e]-a.fc[b]}}(e));m=$jscomp.makeIterator(m);for(h=m.next();!h.done;h=m.next()){h=h.value;var l=encodeURI(this.homepages[h]),n=this.translateNameToDBLP(h);k+='<tr><td>&nbsp;&nbsp;&nbsp;&nbsp;</td><td><small><a title="Click for author\'s home page." target="_blank" href="'+
l+'" onclick="trackOutboundLink(\''+l+"', true); return false;\">"+h+"</a>&nbsp;";this.turing.hasOwnProperty(h)&&(k+="<b>[Turing Award winner]</b>&nbsp;");k+='<font style="font-variant:small-caps" size="-1">'+this.areaString(h).toLowerCase()+"</em></font>&nbsp;";k+='<a title="Click for author\'s home page." target="_blank" href="'+l+'" onclick="trackOutboundLink(\''+l+'\', true); return false;"><img src="'+this.homepageImage+'"></a>&nbsp;';this.scholarInfo.hasOwnProperty(h)&&"NOSCHOLARPAGE"!=this.scholarInfo[h]&&
...
g[k],h=Math.round(10*this.stats[m])/10;if(99999<=k&&h!=e)break;if(0===h)break;e!=h&&(this.useDenseRankings?f+=1:(f+=a,a=0));e=escape(m);c+="\n<tr><td>"+f+"</td>";c+='<td><span class="hovertip" onclick="csr.toggleFaculty(\''+e+'\')";" id="'+e+'-widget"><font color="blue">'+this.RightTriangle+"</font></span>";c+="&nbsp;"+m+'&nbsp;<font color="blue"><span class="hovertip" onclick="csr.toggleChart(\''+e+'\')";" >'+this.PieChart+"</span></font>";c+="</td>";c+='<td align="right">'+(Math.round(10*h)/10).toFixed(1)+
"</td>";c+='<td align="right">'+b[m]+"<br />";c+="</td>";c+="</tr>\n";c+='<tr><td colspan="4"><div style="display:none;" style="width: 100%; height: 350px;" id="'+e+'-chart"></div></td></tr>';c+='<tr><td colspan="4"><div style="display:none;" id="'+e+'-faculty">'+d[m]+"</div></td></tr>";a++;e=h}c+="</tbody></table><br /></div></div>\n<br></body></html>"}else c="<h3>Please select at least one area by clicking one or more checkboxes.</h3>";return c};
...
CSRankings.prototype.rank=function(a){a=void 0===a?!0:a;var b={},d={},c={},f={},e={};this.areaDeptAdjustedCount={};var g=parseInt(jQuery("#fromyear").find(":selected").text()),k=parseInt(jQuery("#toyear").find(":selected").text()),m=jQuery("#regions").find(":selected").val(),h=this.updateWeights(e);this.authorAreas={};this.countAuthorAreas(g,k);this.buildDepartments(g,k,e,m,d,b,c,f);this.computeStats(b,h,e);this.canonicalizeNames(b,c,f);b=this.buildDropDown(b,c,f);d=this.buildOutputString(h,d,b);

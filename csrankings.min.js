...
a.countAuthorAreas();a.loadCountryInfo(a.countryInfo,function(){setTimeout(function(){a.addListeners();CSRankings.geoCheck()},0)})})})})})})};CSRankings.getInstance=function(){return CSRankings.theInstance};CSRankings.promise=function(a){"undefined"!==typeof Promise?Promise.resolve().then(a):setTimeout(a,0)};
...
CSRankings.prototype.countAuthorAreas=function(){var a=parseInt(jQuery("#fromyear").find(":selected").text()),b=parseInt(jQuery("#toyear").find(":selected").text());this.authorAreas={};for(var d in this.authors)if(this.authors.hasOwnProperty(d)){var c=this.authors[d].area;if(!(c in CSRankings.nextTier)){var e=this.authors[d].year;if(!(e<a||e>b)){var f=this.authors[d];e=f.name;var h=f.dept;f=parseFloat(f.count);if(!(e in this.authorAreas)){this.authorAreas[e]={};for(var k in this.areaDict)this.areaDict.hasOwnProperty(k)&&
(this.authorAreas[e][k]=0)}if(!(h in this.authorAreas)){this.authorAreas[h]={};for(var m in this.areaDict)this.areaDict.hasOwnProperty(m)&&(this.authorAreas[h][m]=0)}this.authorAreas[e][c]+=f;this.authorAreas[h][c]+=f}}}};
...
CSRankings.prototype.rank=function(a){a=void 0===a?!0:a;var b=performance.now(),d={},c={},e={},f={},h={};this.areaDeptAdjustedCount={};var k=parseInt(jQuery("#fromyear").find(":selected").text()),m=parseInt(jQuery("#toyear").find(":selected").text()),g=jQuery("#regions").find(":selected").val(),l=this.updateWeights(h);this.buildDepartments(k,m,h,g,c,d,e,f);this.computeStats(d,l,h);d=this.buildDropDown(d,e,f);c=this.buildOutputString(l,c,d);jQuery("#success").html(c);a?this.navigoRouter.resume():this.navigoRouter.pause();
this.urlUpdate();a=performance.now();console.log("Rank took "+(a-b)+" milliseconds.");return!1};CSRankings.prototype.toggleChart=function(a){var b=document.getElementById(a+"-chart");"block"===b.style.display?(b.style.display="none",b.innerHTML=""):(b.style.display="block",this.makeChart(a))};

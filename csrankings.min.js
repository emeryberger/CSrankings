...
{};for(var m=$jscomp.makeIterator(a[f.dept]),g=m.next();!g.done;g=m.next())g=g.value,f.fc[g]=b[g+f.dept];m=Object.keys(f.fc);m.sort(function(a){return function(b,e){if(a.fc[e]===a.fc[b]){var f=Math.round(10*d[e+a.dept])/10,h=Math.round(10*d[b+a.dept])/10;return f===h?c.compareNames(b,e):f-h}return a.fc[e]-a.fc[b]}}(f));m=$jscomp.makeIterator(m);for(g=m.next();!g.done;g=m.next()){g=g.value;var l=encodeURI(this.homepages[g]),n=this.dblpAuthors[g];k+='<tr><td>&nbsp;&nbsp;&nbsp;&nbsp;</td><td><small><a title="Click for author\'s home page." target="_blank" href="'+
...
CSRankings.prototype.buildOutputString=function(a,b,d){var c=this.makePrologue();c+='<thead><tr><th align="left">Rank&nbsp;&nbsp;</th><th align="right">Institution&nbsp;&nbsp;</th><th align="right"><abbr title="Geometric mean count of papers published across all areas.">Count</abbr></th><th align="right">&nbsp;<abbr title="Number of faculty who have published in these areas.">Faculty</abbr></th></th></tr></thead><tbody>';if(0<a){a=1;var e=0,f=9999999.999;for(h in this.stats)this.stats[h]=Math.round(10*
this.stats[h])/10;var h=this.sortIndex(this.stats);for(var k=0;k<h.length;k++){var m=h[k],g=this.stats[m];if(99999<=k&&g!=f)break;if(0===g)break;f!=g&&(this.useDenseRankings?e+=1:(e+=a,a=0));f=escape(m);c+="\n<tr><td>"+e+"</td>";c+='<td><span class="hovertip" onclick="csr.toggleFaculty(\''+f+'\')";" id="'+f+'-widget"><font color="blue">'+this.RightTriangle+"</font></span>";c+="&nbsp;"+m+'&nbsp;<font color="blue"><span class="hovertip" onclick="csr.toggleChart(\''+f+'\')";" >'+this.PieChart+"</span></font>";
c+="</td>";c+='<td align="right">'+(Math.round(10*g)/10).toFixed(1)+"</td>";c+='<td align="right">'+b[m]+"<br />";c+="</td>";c+="</tr>\n";c+='<tr><td colspan="4"><div style="display:none;" style="width: 100%; height: 350px;" id="'+f+'-chart"></div></td></tr>';c+='<tr><td colspan="4"><div style="display:none;" id="'+f+'-faculty">'+d[m]+"</div></td></tr>";a++;f=g}c+="</tbody></table><br /></div></div>\n<br></body></html>"}else c="<h3>Please select at least one area by clicking one or more checkboxes.</h3>";
...
CSRankings.prototype.rank=function(a){a=void 0===a?!0:a;var b={},d={},c={},e={},f={};this.areaDeptAdjustedCount={};var h=parseInt(jQuery("#fromyear").find(":selected").text()),k=parseInt(jQuery("#toyear").find(":selected").text()),m=jQuery("#regions").find(":selected").val(),g=this.updateWeights(f);this.buildDepartments(h,k,f,m,d,b,c,e);this.computeStats(b,g,f);b=this.buildDropDown(b,c,e);d=this.buildOutputString(g,d,b);jQuery("#success").html(d);a?this.navigoRouter.resume():this.navigoRouter.pause();
a=this.updatedURL();this.navigoRouter.navigate(a);return!1};CSRankings.prototype.toggleChart=function(a){var b=document.getElementById(a+"-chart");"block"===b.style.display?(b.style.display="none",b.innerHTML=""):(b.style.display="block",this.makeChart(a))};

...
"&#9658;";this.DownTriangle="&#9660;";this.PieChart="&#9685;";CSRankings.theInstance=this;this.navigoRouter=new Navigo(null,!0);for(var b=0;b<this.areaMap.length;b++){var d=this.areaMap[b],c=d.area;d=d.title;CSRankings.areas[b]=c;this.areaNames[b]=d;this.fields[b]=c;this.areaDict[c]=d;this.areaPosition[c]=b}c=$jscomp.makeIterator(this.aiAreas);for(b=c.next();!b.done;b=c.next())this.aiFields.push(this.areaPosition[b.value]);c=$jscomp.makeIterator(this.systemsAreas);for(b=c.next();!b.done;b=c.next())this.systemsFields.push(this.areaPosition[b.value]);
c=$jscomp.makeIterator(this.theoryAreas);for(b=c.next();!b.done;b=c.next())this.theoryFields.push(this.areaPosition[b.value]);c=$jscomp.makeIterator(this.interdisciplinaryAreas);for(b=c.next();!b.done;b=c.next())this.otherFields.push(this.areaPosition[b.value]);b=0;for(var f in CSRankings.parentMap)c=CSRankings.parentMap[f],c in CSRankings.childMap?CSRankings.childMap[c].push(f):(CSRankings.childMap[c]=[f],CSRankings.parentIndex[c]=b,b+=1);this.displayProgress(1);this.loadAliases(this.aliases,function(){a.loadTuring(a.turing,
function(){a.displayProgress(2);a.loadAuthorInfo(function(){a.displayProgress(3);a.loadAuthors(function(){a.setAllOn();a.navigoRouter.on({"/index":a.navigation,"/fromyear/:fromyear/toyear/:toyear/index":a.navigation}).resolve();a.displayProgress(4);a.loadCountryInfo(a.countryInfo,function(){setTimeout(function(){a.addListeners();CSRankings.geoCheck()},0)})})})})})};CSRankings.getInstance=function(){return CSRankings.theInstance};
...
CSRankings.average=function(a){return CSRankings.sum(a)/a.length};
CSRankings.prototype.areaString=function(a){if(!this.authorAreas[a])return"";for(var b={},d=CSRankings.areas,c=0,f=0;f<d.length;f++){var e=d[f];if(!(e in CSRankings.nextTier)){var h=this.authorAreas[a][e];e in CSRankings.parentMap&&(e=this.areaDict[e]);0<h&&(e in b||(b[e]=0),b[e]+=h,c=b[e]>c?b[e]:c)}}a=[];for(var k in b)a.push(b[k]);var m=CSRankings.average(a);d=a.map(function(a){a-=m;return a*a});k=0;1<a.length&&(k=Math.ceil(Math.sqrt(CSRankings.sum(d)/(a.length-1))));a=[];for(var g in b)b[g]>=c-
k&&a.push(g);return a.sort(function(a,c){return b[c]-b[a]}).slice(0,3).join(",")};CSRankings.prototype.compareNames=function(a,b){a=a.split(" ");b=b.split(" ");a=a[a.length-1];b=b[b.length-1];return a<b?-1:a>b?1:0};
CSRankings.prototype.makeChart=function(a){for(var b=[],d={},c=CSRankings.areas,f=unescape(a),e=0;e<c.length;e++){var h=c[e];if(!(f in this.authorAreas))return;if(!(h in CSRankings.nextTier)){var k=this.authorAreas[f][h];k=Math.round(10*k)/10;0<k&&(h in CSRankings.parentMap&&(h=CSRankings.parentMap[h]),h in d||(d[h]=0),d[h]+=k)}}for(var m in d)b.push({label:this.areaDict[m],value:Math.round(10*d[m])/10,color:this.color[CSRankings.parentIndex[m]]});new d3pie(a+"-chart",{header:{title:{text:f,fontSize:24,
...
CSRankings.prototype.loadAliases=function(a,b){Papa.parse(this.aliasFile,{header:!0,download:!0,complete:function(d){d=$jscomp.makeIterator(d.data);for(var c=d.next();!c.done;c=d.next())c=c.value,a[c.alias]=c.name;setTimeout(b,0)}})};CSRankings.prototype.loadTuring=function(a,b){Papa.parse(this.turingFile,{header:!0,download:!0,complete:function(d){d=$jscomp.makeIterator(d.data);for(var c=d.next();!c.done;c=d.next())c=c.value,a[c.name]=c.year;setTimeout(b,0)}})};
CSRankings.prototype.loadCountryInfo=function(a,b){Papa.parse(this.countryinfoFile,{header:!0,download:!0,complete:function(d){d=$jscomp.makeIterator(d.data);for(var c=d.next();!c.done;c=d.next())c=c.value,a[c.institution]=c.region;setTimeout(b,0)}})};
CSRankings.prototype.loadAuthorInfo=function(a){var b=this;Papa.parse(this.authorFile,{download:!0,header:!0,complete:function(d){d=d.data;for(var c=0;c<d.length;c++){var f=d[c],e=f.name;b.homepages[e.trim()]=f.homepage;b.scholarInfo[e.trim()]=f.scholarid}setTimeout(a,0)}})};CSRankings.prototype.loadAuthors=function(a){var b=this;Papa.parse(this.authorinfoFile,{download:!0,header:!0,complete:function(d){b.authors=d.data;setTimeout(a,0)}})};
...
CSRankings.prototype.countAuthorAreas=function(a,b){for(var d in this.authors)if(this.authors.hasOwnProperty(d)){var c=this.authors[d],f=c.year;if(!(f<a||f>b||(f=c.area,f in CSRankings.nextTier))){var e=c.dept,h=parseFloat(c.count);c=c.name;c in this.aliases&&(c=this.aliases[c]);if(!(c in this.authorAreas)){this.authorAreas[c]={};for(var k in this.areaDict)this.areaDict.hasOwnProperty(k)&&(this.authorAreas[c][k]=0)}if(!(e in this.authorAreas)){this.authorAreas[e]={};for(var m in this.areaDict)this.areaDict.hasOwnProperty(m)&&
(this.authorAreas[e][m]=0)}this.authorAreas[c][f]+=h;this.authorAreas[e][f]+=h}}};
CSRankings.prototype.buildDepartments=function(a,b,d,c,f,e,h,k){var m={},g;for(g in this.authors)if(this.authors.hasOwnProperty(g)){var l=this.authors[g],n=l.name,q=l.year,p=l.area;l=l.dept;n in this.aliases&&(n=this.aliases[n]);if("undefined"!==typeof l&&!(0===d[p]||q<a||q>b)&&this.inRegion(l,c)){p in CSRankings.parentMap&&(p=CSRankings.parentMap[p]);q=p+l;p=n+l;q in this.areaDeptAdjustedCount||(this.areaDeptAdjustedCount[q]=0);var t=parseInt(this.authors[g].count),r=parseFloat(this.authors[g].adjustedcount);
this.areaDeptAdjustedCount[q]+=r;n in m||(m[n]=!0,h[p]=0,k[p]=0,l in f||(f[l]=0,e[l]=[]),e[l].push(n),f[l]+=1);h[p]+=t;k[p]+=r}}};
...
{};for(var m=$jscomp.makeIterator(a[e.dept]),g=m.next();!g.done;g=m.next())g=g.value,e.fc[g]=b[g+e.dept];m=Object.keys(e.fc);m.sort(function(a){return function(b,e){if(a.fc[e]===a.fc[b]){var f=Math.round(10*d[e+a.dept])/10,h=Math.round(10*d[b+a.dept])/10;return f===h?c.compareNames(b,e):f-h}return a.fc[e]-a.fc[b]}}(e));m=$jscomp.makeIterator(m);for(g=m.next();!g.done;g=m.next()){g=g.value;var l=encodeURI(this.homepages[g]),n=this.translateNameToDBLP(g);k+='<tr><td>&nbsp;&nbsp;&nbsp;&nbsp;</td><td><small><a title="Click for author\'s home page." target="_blank" href="'+
l+'" onclick="trackOutboundLink(\''+l+"', true); return false;\">"+g+"</a>&nbsp;";this.turing.hasOwnProperty(g)&&(k+="<b>[Turing Award winner]</b>&nbsp;");k+='<font style="font-variant:small-caps" size="-1">'+this.areaString(g).toLowerCase()+"</em></font>&nbsp;";k+='<a title="Click for author\'s home page." target="_blank" href="'+l+'" onclick="trackOutboundLink(\''+l+'\', true); return false;"><img src="'+this.homepageImage+'"></a>&nbsp;';this.scholarInfo.hasOwnProperty(g)&&"NOSCHOLARPAGE"!=this.scholarInfo[g]&&
(l="https://scholar.google.com/citations?user="+this.scholarInfo[g]+"&hl=en&oi=ao",k+='<a title="Click for author\'s Google Scholar page." target="_blank" href="'+l+'" onclick="trackOutboundLink(\''+l+'\', true); return false;"><img src="scholar-favicon.ico" height="10" width="10"></a>&nbsp;');k+='<a title="Click for author\'s DBLP entry." target="_blank" href="'+n+'" onclick="trackOutboundLink(\''+n+'\', true); return false;"><img src="dblp.png"></a>';k+="<span onclick='csr.toggleChart(\""+escape(g)+
'");\' title="Click for author\'s publication profile." class="hovertip" ><font size="+1" color="blue">'+this.PieChart+'</font></span></small></td><td align="right"><small><a title="Click for author\'s DBLP entry." target="_blank" href="'+n+'" onclick="trackOutboundLink(\''+n+"', true); return false;\">"+e.fc[g]+'</a></small></td><td align="right"><small>'+(Math.round(10*d[g+e.dept])/10).toFixed(1)+'</small></td></tr><tr><td colspan="4"><div style="display:none;" id="'+escape(g)+'-chart"></div></td></tr>'}k+=
...
h[k],g=Math.round(10*this.stats[m])/10;if(99999<=k&&g!=e)break;if(0===g)break;e!=g&&(this.useDenseRankings?f+=1:(f+=a,a=0));e=escape(m);c+="\n<tr><td>"+f+"</td>";c+='<td><span class="hovertip" onclick="csr.toggleFaculty(\''+e+'\')";" id="'+e+'-widget"><font color="blue">'+this.RightTriangle+"</font></span>";c+="&nbsp;"+m+'&nbsp;<font color="blue"><span class="hovertip" onclick="csr.toggleChart(\''+e+'\')";" >'+this.PieChart+"</span></font>";c+="</td>";c+='<td align="right">'+(Math.round(10*g)/10).toFixed(1)+
"</td>";c+='<td align="right">'+b[m]+"<br />";c+="</td>";c+="</tr>\n";c+='<tr><td colspan="4"><div style="display:none;" style="width: 100%; height: 350px;" id="'+e+'-chart"></div></td></tr>';c+='<tr><td colspan="4"><div style="display:none;" id="'+e+'-faculty">'+d[m]+"</div></td></tr>";a++;e=g}c+="</tbody></table><br /></div></div>\n<br></body></html>"}else c="<h3>Please select at least one area by clicking one or more checkboxes.</h3>";return c};
...
CSRankings.prototype.rank=function(a){a=void 0===a?!0:a;var b={},d={},c={},f={},e={};this.areaDeptAdjustedCount={};var h=parseInt(jQuery("#fromyear").find(":selected").text()),k=parseInt(jQuery("#toyear").find(":selected").text()),m=jQuery("#regions").find(":selected").val(),g=this.updateWeights(e);this.authorAreas={};this.countAuthorAreas(h,k);this.buildDepartments(h,k,e,m,d,b,c,f);this.computeStats(b,g,e);this.canonicalizeNames(b,c,f);b=this.buildDropDown(b,c,f);d=this.buildOutputString(g,d,b);

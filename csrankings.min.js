...
CSRankings.prototype.areaString=function(a){if(!this.authorAreas[a])return"";for(var b={},d=CSRankings.areas,c=0,f=0;f<d.length;f++){var e=d[f];if(!(e in CSRankings.nextTier)){var h=this.authorAreas[a][e];e in CSRankings.parentMap&&(e=this.areaDict[e]);0<h&&(e in b||(b[e]=0),b[e]+=h,c=b[e]>c?b[e]:c)}}a=[];for(var k in b)a.push(b[k]);k=CSRankings.sum(a);d=0;1<a.length&&(d=Math.ceil(1*CSRankings.stddev(a)));a=[];for(var l in b)b[l]>=c-d&&.2<=1*b[l]/k&&1<b[l]&&a.push(l);return a.sort(function(a,c){return b[c]-
...
CSRankings.prototype.makeChart=function(a){for(var b=[],d={},c=CSRankings.areas,f=unescape(a),e=0;e<c.length;e++){var h=c[e];if(!(f in this.authorAreas))return;if(!(h in CSRankings.nextTier)){var k=this.authorAreas[f][h];k=Math.round(10*k)/10;0<k&&(h in CSRankings.parentMap&&(h=CSRankings.parentMap[h]),h in d||(d[h]=0),d[h]+=k)}}for(var l in d)b.push({label:this.areaDict[l],value:Math.round(10*d[l])/10,color:this.color[CSRankings.parentIndex[l]]});new d3pie(a+"-chart",{header:{title:{text:f,fontSize:24,
...
CSRankings.prototype.loadAuthorInfo=function(a){var b=this;Papa.parse(this.authorFile,{download:!0,header:!0,complete:function(d){d=d.data;for(var c=0;c<d.length;c++){var f=d[c],e=f.name.trim();""!==e&&(b.dblpAuthors[e]=b.translateNameToDBLP(e),b.homepages[e]=f.homepage,b.scholarInfo[e]=f.scholarid)}CSRankings.promise(a)}})};
CSRankings.prototype.loadAuthors=function(a){var b=this;Papa.parse(this.authorinfoFile,{download:!0,header:!0,complete:function(d){b.authors=d.data;for(var c in b.authors){var f=b.authors[c];d=f.name;var e=f.area,h=f.dept,k=f.subarea,l=f.count,g=f.adjustedcount;f=f.year;d in b.aliases&&(d=b.aliases[d],b.authors[c]={name:d,area:e,dept:h,subarea:k,count:l,adjustedcount:g,year:f})}CSRankings.promise(a)}})};
...
CSRankings.prototype.countAuthorAreas=function(a,b){for(var d in this.authors)if(this.authors.hasOwnProperty(d)){var c=this.authors[d],f=c.name,e=c.year,h=c.area,k=c.dept;if(!(e<a||e>b||h in CSRankings.nextTier)){c=parseFloat(c.count);if(!(f in this.authorAreas)){this.authorAreas[f]={};for(var l in this.areaDict)this.areaDict.hasOwnProperty(l)&&(this.authorAreas[f][l]=0)}if(!(k in this.authorAreas)){this.authorAreas[k]={};for(var g in this.areaDict)this.areaDict.hasOwnProperty(g)&&(this.authorAreas[k][g]=
0)}this.authorAreas[f][h]+=c;this.authorAreas[k][h]+=c}}};
CSRankings.prototype.buildDepartments=function(a,b,d,c,f,e,h,k){var l={},g;for(g in this.authors)if(this.authors.hasOwnProperty(g)){var m=this.authors[g],n=m.name,q=m.year,p=m.area;m=m.dept;if(!(0===d[p]||q<a||q>b)&&this.inRegion(m,c)&&"undefined"!==typeof m){p in CSRankings.parentMap&&(p=CSRankings.parentMap[p]);q=p+m;p=n+m;q in this.areaDeptAdjustedCount||(this.areaDeptAdjustedCount[q]=0);var t=parseInt(this.authors[g].count),r=parseFloat(this.authors[g].adjustedcount);this.areaDeptAdjustedCount[q]+=
r;n in l||(l[n]=!0,h[p]=0,k[p]=0,m in f||(f[m]=0,e[m]=[]),e[m].push(n),f[m]+=1);h[p]+=t;k[p]+=r}}};
CSRankings.prototype.computeStats=function(a,b,d){this.stats={};for(var c in a)if(a.hasOwnProperty(c)){this.stats[c]=1;for(var f=$jscomp.makeIterator(CSRankings.areas),e=f.next();!e.done;e=f.next())if(e=e.value,!(e in CSRankings.parentMap)){var h=e+c;h in this.areaDeptAdjustedCount||(this.areaDeptAdjustedCount[h]=0);0!=d[e]&&(this.stats[c]*=this.areaDeptAdjustedCount[h]+1)}this.stats[c]=Math.pow(this.stats[c],1/b)}};
...
CSRankings.prototype.buildDropDown=function(a,b,d){var c=this,f={},e={},h;for(h in a)if(e.dept=h,a.hasOwnProperty(e.dept)){var k='<div class="table"><table class="table table-sm table-striped"><thead><th></th><td><small><em><abbr title="Click on an author\'s name to go to their home page.">Faculty</abbr></em></small></td><td align="right"><small><em>&nbsp;&nbsp;<abbr title="Total number of publications (click for DBLP entry).">#&nbsp;Pubs</abbr></em></small></td><td align="right"><small><em><abbr title="Count divided by number of co-authors">Adj.&nbsp;#</abbr></em></small></td></thead><tbody>';e.fc=
{};for(var l=$jscomp.makeIterator(a[e.dept]),g=l.next();!g.done;g=l.next())g=g.value,e.fc[g]=b[g+e.dept];l=Object.keys(e.fc);l.sort(function(a){return function(b,e){if(a.fc[e]===a.fc[b]){var f=Math.round(10*d[e+a.dept])/10,h=Math.round(10*d[b+a.dept])/10;return f===h?c.compareNames(b,e):f-h}return a.fc[e]-a.fc[b]}}(e));l=$jscomp.makeIterator(l);for(g=l.next();!g.done;g=l.next()){g=g.value;var m=encodeURI(this.homepages[g]),n=this.dblpAuthors[g];k+='<tr><td>&nbsp;&nbsp;&nbsp;&nbsp;</td><td><small><a title="Click for author\'s home page." target="_blank" href="'+
m+'" onclick="trackOutboundLink(\''+m+"', true); return false;\">"+g+"</a>&nbsp;";this.acmfellow.hasOwnProperty(g)&&(k+='<span title="ACM Fellow"><img src="'+this.acmfellowImage+'"></span>&nbsp;');this.turing.hasOwnProperty(g)&&(k+='<span title="Turing Award"><img src="'+this.turingImage+'"></span>&nbsp;');k+='<font style="font-variant:small-caps" size="-1">'+this.areaString(g).toLowerCase()+"</em></font>&nbsp;";k+='<a title="Click for author\'s home page." target="_blank" href="'+m+'" onclick="trackOutboundLink(\''+
m+'\', true); return false;"><img src="'+this.homepageImage+'"></a>&nbsp;';this.scholarInfo.hasOwnProperty(g)&&"NOSCHOLARPAGE"!=this.scholarInfo[g]&&(m="https://scholar.google.com/citations?user="+this.scholarInfo[g]+"&hl=en&oi=ao",k+='<a title="Click for author\'s Google Scholar page." target="_blank" href="'+m+'" onclick="trackOutboundLink(\''+m+'\', true); return false;"><img src="scholar-favicon.ico" height="10" width="10"></a>&nbsp;');k+='<a title="Click for author\'s DBLP entry." target="_blank" href="'+
n+'" onclick="trackOutboundLink(\''+n+'\', true); return false;"><img src="dblp.png"></a>';k+="<span onclick='csr.toggleChart(\""+escape(g)+'");\' title="Click for author\'s publication profile." class="hovertip" ><font size="+1" color="blue">'+this.PieChart+'</font></span></small></td><td align="right"><small><a title="Click for author\'s DBLP entry." target="_blank" href="'+n+'" onclick="trackOutboundLink(\''+n+"', true); return false;\">"+e.fc[g]+'</a></small></td><td align="right"><small>'+(Math.round(10*
d[g+e.dept])/10).toFixed(1)+'</small></td></tr><tr><td colspan="4"><div style="display:none;" id="'+escape(g)+'-chart"></div></td></tr>'}k+="</tbody></table></div>";f[e.dept]=k;e={fc:e.fc,dept:e.dept}}return f};
CSRankings.prototype.buildOutputString=function(a,b,d){var c=this.makePrologue();c+='<thead><tr><th align="left">Rank&nbsp;&nbsp;</th><th align="right">Institution&nbsp;&nbsp;</th><th align="right"><abbr title="Geometric mean count of papers published across all areas.">Count</abbr></th><th align="right">&nbsp;<abbr title="Number of faculty who have published in these areas.">Faculty</abbr></th></th></tr></thead><tbody>';if(0<a){a=1;for(var f=0,e=9999999.999,h=this.sortIndex(this.stats),k=0;k<h.length;k++){var l=
h[k],g=Math.round(10*this.stats[l])/10;if(99999<=k&&g!=e)break;if(0===g)break;e!=g&&(this.useDenseRankings?f+=1:(f+=a,a=0));e=escape(l);c+="\n<tr><td>"+f+"</td>";c+='<td><span class="hovertip" onclick="csr.toggleFaculty(\''+e+'\')";" id="'+e+'-widget"><font color="blue">'+this.RightTriangle+"</font></span>";c+="&nbsp;"+l+'&nbsp;<font color="blue"><span class="hovertip" onclick="csr.toggleChart(\''+e+'\')";" >'+this.PieChart+"</span></font>";c+="</td>";c+='<td align="right">'+(Math.round(10*g)/10).toFixed(1)+
"</td>";c+='<td align="right">'+b[l]+"<br />";c+="</td>";c+="</tr>\n";c+='<tr><td colspan="4"><div style="display:none;" style="width: 100%; height: 350px;" id="'+e+'-chart"></div></td></tr>';c+='<tr><td colspan="4"><div style="display:none;" id="'+e+'-faculty">'+d[l]+"</div></td></tr>";a++;e=g}c+="</tbody></table><br /></div></div>\n<br></body></html>"}else c="<h3>Please select at least one area by clicking one or more checkboxes.</h3>";return c};
...
CSRankings.prototype.rank=function(a){a=void 0===a?!0:a;var b={},d={},c={},f={},e={};this.areaDeptAdjustedCount={};var h=parseInt(jQuery("#fromyear").find(":selected").text()),k=parseInt(jQuery("#toyear").find(":selected").text()),l=jQuery("#regions").find(":selected").val(),g=this.updateWeights(e);this.authorAreas={};this.countAuthorAreas(h,k);this.buildDepartments(h,k,e,l,d,b,c,f);this.computeStats(b,g,e);this.canonicalizeNames(b,c,f);b=this.buildDropDown(b,c,f);d=this.buildOutputString(g,d,b);
...
(c?0:1):a.allChecked&c}}(c)),c.allChecked&&(a+=this.fields[f]+"&",b+=1))}0<b&&(a=a.slice(0,-1));c=jQuery("#regions").find(":selected").val();f="";e=(new Date).getFullYear();var h=parseInt(jQuery("#fromyear").find(":selected").text()),k=parseInt(jQuery("#toyear").find(":selected").text());if(h!=e-10||k!=e)f+="/fromyear/"+h.toString(),f+="/toyear/"+k.toString();f=b==d?f+"/index?all":0==b?f+"/index?none":f+("/index?"+a);"USA"!=c&&(f+="&"+c);this.navigoRouter.navigate(f)};

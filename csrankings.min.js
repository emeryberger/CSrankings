...
CSRankings.prototype.makeBarChart=function(a){var b=this,c=[],d={},f=CSRankings.topTierAreas,h=unescape(a),e=[].concat($jscomp.arrayFromIterable(this.aiAreas.map(function(k){return{key:k,label:b.areaDict[k],color:"#377eb8"}})),$jscomp.arrayFromIterable(this.systemsAreas.map(function(k){return{key:k,label:b.areaDict[k],color:"#ff7f00"}})),$jscomp.arrayFromIterable(this.theoryAreas.map(function(k){return{key:k,label:b.areaDict[k],color:"#4daf4a"}})),$jscomp.arrayFromIterable(this.interdisciplinaryAreas.map(function(k){return{key:k,
label:b.areaDict[k],color:"#984ea3"}})));e.forEach(function(k){return d[k.key]=0});for(var g in f){if(!(h in this.authorAreas))return;f=this.authorAreas[h][g];f=Math.round(10*f)/10;0<f&&(g in CSRankings.parentMap&&(g=CSRankings.parentMap[g]),d[g]+=f)}e.forEach(function(k){c.push({area:b.areaDict[k.key],value:Math.round(10*d[k.key])/10});k.label=b.areaDict[k.key]});h=e.sort(function(k,l){return k.label>l.label?1:k.label<l.label?-1:0}).map(function(k){return k.color});vegaEmbed('div[id="'+a+'-chart"]',
{$schema:"https://vega.github.io/schema/vega-lite/v5.json",data:{values:c},mark:"bar",encoding:{x:{field:"area",type:"nominal",sort:null,axis:{title:null}},y:{field:"value",type:"quantitative",axis:{title:null}},tooltip:[{field:"area",type:"nominal",title:"Area"},{field:"value",type:"quantitative",title:"Count"}],color:{field:"area",type:"nominal",scale:{range:h},legend:null}},width:420,height:80,padding:{left:25,top:3}},{actions:!1})};
...
'</span></small></td><td align="right"><small><a title="Click for author\'s DBLP entry." target="_blank" href="'+n+'" onclick="trackOutboundLink(\''+n+"', true); return false;\">"+h.$jscomp$loop$prop$fc$28[l]+'</a></small></td><td align="right"><small>'+(Math.round(10*c[l])/10).toFixed(1)+'</small></td></tr><tr><td colspan="4"><div class="csr-chart" id="'+escape(l)+'-chart"></div></td></tr>'}g+="</tbody></table></div>";f[e]=g}h={$jscomp$loop$prop$fc$28:h.$jscomp$loop$prop$fc$28}}return f};
...
m in b&&(p=b[m]);h+="&nbsp;"+m+('&nbsp;<img src="/flags/'+p+'.png">&nbsp;<span class="hovertip" onclick="csr.toggleChart(\'')+g+'\');" id="'+g+'-chartwidget">'+this.BarChart+"</span>";h+="</td>";h+='<td align="right">'+(Math.round(10*n)/10).toFixed(1)+"</td>";h+='<td align="right">'+c[m];h+="</td>";h+="</tr>\n";h+='<tr><td colspan="4"><div class="csr-chart" id="'+g+'-chart"></div></td></tr>';h+='<tr><td colspan="4"><div style="display:none;" id="'+g+'-faculty">'+d[m]+"</div></td></tr>";a++;g=n}h+=
...
CSRankings.prototype.toggleChart=function(a){var b=document.getElementById(a+"-chart"),c=document.getElementById(a+"-chartwidget");"block"===b.style.display?(b.style.display="none",b.innerHTML="",c.innerHTML=this.BarChart):(b.style.display="block",this.usePieChart?this.makePieChart(a):this.makeBarChart(a),c.innerHTML=this.OpenBarChart)};
...
0==b?f+"/index?none":f+("/index?"+a);"USA"!=d&&(f+="&"+d);"pie"==$("#charttype").find(":selected").val()?(this.usePieChart=!0,f+="&pie"):this.usePieChart=!1;return f};CSRankings.geoCheck=function(){navigator.geolocation.getCurrentPosition(function(a){a=whichContinent(a.coords.latitude,a.coords.longitude);var b=document.getElementById("regions");switch(a){case "northamerica":return;case "europe":case "asia":case "southamerica":case "africa":b.value=a;break;default:b.value="world"}CSRankings.getInstance().rank()})};
CSRankings.prototype.navigation=function(a,b){null!==a&&Object.keys(a).forEach(function(e){$("#"+e).prop("value",a[e].toString())});CSRankings.clearNonSubsetted();var c=b.split("&");b=c.some(function(e){return"all"==e});var d=c.some(function(e){return"none"==e});if(c.some(function(e){return 0<=CSRankings.regions.indexOf(e)})){var f=0;c.forEach(function(e){0<=CSRankings.regions.indexOf(e)&&c.splice(f,1);$("#regions").val(e);f+=1})}c.some(function(e){return"pie"==e})&&$("#charttype").val("pie");if(b)for(var h in CSRankings.topTierAreas)b=
"input[name="+h+"]",$(b).prop("checked",!0),h in CSRankings.childMap&&($(b).prop("disabled",!1),CSRankings.childMap[h].forEach(function(e){e in CSRankings.nextTier||$("input[name="+e+"]").prop("checked",!0)}));else if(d)CSRankings.clearNonSubsetted();else for(CSRankings.clearNonSubsetted(),h=$jscomp.makeIterator(c),b=h.next();!b.done;b=h.next())b=b.value,"none"!=b&&""!=b&&(d="input[name="+b+"]",$(d).prop("checked",!0),$(d).prop("disabled",!1),b in CSRankings.childMap&&CSRankings.childMap[b].forEach(function(e){e in
...
CSRankings.prototype.addListeners=function(){var a=this;["toyear","fromyear","regions","charttype"].forEach(function(e){document.getElementById(e).addEventListener("change",function(){a.countAuthorAreas();a.rank()})});for(var b={},c=0;c<CSRankings.areas.length;b={$jscomp$loop$prop$area$32:b.$jscomp$loop$prop$area$32},c++)if(b.$jscomp$loop$prop$area$32=CSRankings.areas[c],!(b.$jscomp$loop$prop$area$32 in CSRankings.parentMap)){var d=document.getElementById(b.$jscomp$loop$prop$area$32+"-widget");d&&
d.addEventListener("click",function(e){return function(){a.toggleConferences(e.$jscomp$loop$prop$area$32)}}(b))}b={};for(c=0;c<this.fields.length;b={$jscomp$loop$prop$field$34:b.$jscomp$loop$prop$field$34,$jscomp$loop$prop$str$35:b.$jscomp$loop$prop$str$35},c++)b.$jscomp$loop$prop$str$35="input[name="+this.fields[c]+"]",b.$jscomp$loop$prop$field$34=this.fields[c],(d=document.getElementById(this.fields[c]))&&d.addEventListener("click",function(e){return function(){var g=!0;if(e.$jscomp$loop$prop$field$34 in
CSRankings.parentMap){g=!1;var k=CSRankings.parentMap[e.$jscomp$loop$prop$field$34],l="input[name="+k+"]",m=0,n=1;CSRankings.childMap[k].forEach(function(r){var t=$("input[name="+r+"]").prop("checked");m|=t;n=r in CSRankings.nextTier?n&(t?0:1):n&t});$(l).prop("checked",m);m&&!n||$(l).prop("disabled",!1);m&&!n&&$(l).prop("disabled",!0)}else if(k=$(e.$jscomp$loop$prop$str$35).prop("checked"),e.$jscomp$loop$prop$field$34 in CSRankings.childMap){l=$jscomp.makeIterator(CSRankings.childMap[e.$jscomp$loop$prop$field$34]);
for(var p=l.next();!p.done;p=l.next()){p=p.value;var q="input[name="+p+"]";p in CSRankings.nextTier?$(q).prop("checked",!1):$(q).prop("checked",k)}}a.rank(g)}}(b));var f={all_areas_on:function(){a.activateAll()},all_areas_off:function(){a.activateNone()},ai_areas_on:function(){a.activateAI()},ai_areas_off:function(){a.deactivateAI()},systems_areas_on:function(){a.activateSystems()},systems_areas_off:function(){a.deactivateSystems()},theory_areas_on:function(){a.activateTheory()},theory_areas_off:function(){a.deactivateTheory()},

...
value:Math.round(10*b[h.key])/10,ratio:b[h.key]/l});h.label=this.areaDict[h.key]});f=f.sort((h,k)=>h.label>k.label?1:h.label<k.label?-1:0).map(h=>h.color);g={$schema:"https://vega.github.io/schema/vega-lite/v5.json",data:{values:d},mark:"bar",encoding:{x:{field:"area",type:"nominal",sort:null,axis:{title:null}},y:{field:"value",type:"quantitative",axis:{title:null}},tooltip:[{field:"area",type:"nominal",title:"Area"},{field:"value",type:"quantitative",title:"Count"}],color:{field:"area",type:"nominal",
scale:{range:f},legend:null}},width:420,height:80,padding:{left:25,top:3}};f={$schema:"https://vega.github.io/schema/vega-lite/v5.json",data:{values:d},encoding:{theta:{field:"value",type:"quantitative",stack:!0},color:{field:"area",type:"nominal",scale:{range:f},legend:null},order:{field:"index"},tooltip:[{field:"area",type:"nominal",title:"Area"},{field:"value",type:"quantitative",title:"Count"},{field:"ratio",type:"quantitative",title:"Ratio",format:".1%"}]},layer:[{mark:{type:"arc",outerRadius:90,
stroke:"#fdfdfd",strokeWidth:1}},{mark:{type:"text",radius:108,dy:-3},encoding:{text:{field:"area",type:"nominal"},color:{condition:{test:"datum.ratio < 0.03",value:"rgba(255, 255, 255, 0)"},field:"area",type:"nominal",scale:{range:f}}}},{mark:{type:"text",radius:108,fontSize:9,dy:7},encoding:{text:{field:"value",type:"quantitative"},color:{condition:{test:"datum.ratio < 0.03",value:"rgba(255, 255, 255, 0)"},field:"area",type:"nominal",scale:{range:f}}}}],width:400,height:250,padding:{left:25,top:3}};
...
this.authors[b];if(e in CSRankings.nextTier)continue;var {year:d}=this.authors[b];if(d<a||d>c)continue;const {name:g,dept:f,count:l}=this.authors[b];d=parseFloat(l);if(!(g in this.authorAreas)){this.authorAreas[g]={};for(const h in this.areaDict)this.areaDict.hasOwnProperty(h)&&(this.authorAreas[g][h]=0)}if(!(f in this.authorAreas)){this.authorAreas[f]={};for(const h in this.areaDict)this.areaDict.hasOwnProperty(h)&&(this.authorAreas[f][h]=0)}this.authorAreas[g][e]+=d;this.authorAreas[f][e]+=d}}buildDepartments(a,
